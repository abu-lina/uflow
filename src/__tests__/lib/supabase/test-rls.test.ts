...
async function testRLS() {
  ...
  const testProfileId =
    profilesData && profilesData.length > 0 ? profilesData[0].id : 'no-profile-found';
  ...
  const otherProfileId =
    profilesData && profilesData.length > 0 && profilesData[0].id !== userId
      ? profilesData[0].id
      : 'some-other-id';
  ...
  if (otherProfileId && otherProfileId !== 'some-other-id') {
    ...
    if (otherUpdateError) {
      console.log(
        '✅ Authenticated UPDATE of another profile correctly failed:',
        otherUpdateError.message
      );
    } else {
      console.error(
        '❌ Authenticated UPDATE of another profile incorrectly succeeded:',
        otherUpdateData
      );
    }
  } else {
