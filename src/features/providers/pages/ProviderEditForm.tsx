...
export function ProviderEditForm({
  ...
  // Extract extension data from food_providers/store_providers joined relations
  const providerAny = provider as unknown as Record<string, unknown>;
  const foodExt = providerAny.food_providers as Record<string, unknown> | null | undefined;
  const storeExt = providerAny.store_providers as Record<string, unknown> | null | undefined;
  const ext = foodExt || storeExt;

  // Initialize form data from provider
  const [formData, setFormData] = useState<ProviderEditFormData>({
    providerName: provider.provider_name || '',
    providerDescription: (providerAny.provider_description as string) || provider.description || '',
    ...
    locations: [],
    openingHours:
      (providerAny.opening_hours as Record<
        string,
    ...
    womenFriendly: (providerAny.women_friendly as boolean) ?? false,
    childrenFriendly: (providerAny.children_friendly as boolean) ?? false,
    makesDonations: (providerAny.makes_donations as boolean) ?? false,
    hasParking: (providerAny.has_parking as boolean) ?? false,
    economicSolidarity: (providerAny.economic_solidarity as boolean) ?? false,
  });
  ...
  const syncFromLocalStorage = useCallback(() => {
    ...
    if (storedSocial) {
      ...
      setFormData((prev) =>
        JSON.stringify(prev.selectedCommunityServiceIds) !== storedSocial
          ? { ...prev, selectedCommunityServiceIds: parsed }
          : prev,
      );
    }
    ...
    if (storedImages) {
      setFormData((prev) =>
        prev.images !== storedImages ? { ...prev, images: storedImages } : prev,
      );
    }
    ...
    if (storedInline) {
      try {
        ...
        if (
          parsed.isOnlineBusiness === true &&
          (parsed.city || provider.address_city || parsed.zipCode || provider.address_zip)
        ) {
  ...
  useEffect(() => {
    ...
    const loadCategories = async () => {
      try {
        const { data, error } = await supabase.from('categories').select('*').order('name_de');
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          address_street:
            submitData.isOnlineBusiness && !submitData.street ? null : submitData.street || null,
          address_zip:
            submitData.isOnlineBusiness && !submitData.zipCode ? null : submitData.zipCode || null,
          address_city:
            submitData.isOnlineBusiness && !submitData.city ? null : submitData.city || null,
          address_country:
            submitData.isOnlineBusiness && !submitData.country ? null : submitData.country || null,
          show_address:
            submitData.isOnlineBusiness && !submitData.city ? false : submitData.showAddress,
      ...
      if (
        ...
        submitData.selectedCommunityServiceIds.length > 0
      ) {
  ...
  return (
    <form ref={formRef} className="flex flex-1 flex-col" onSubmit={handleSubmit}>
      ...
      <div className="flex flex-1 flex-col gap-8 pb-24 pt-8">
        {/* Basics Section */}
        <div className="flex flex-col gap-4">
          ...
          {expandedSections.basics && (
            <div className="space-y-3">
              ...
              {/* Description Field */}
              <div className="flex min-h-[80px] w-full rounded-2xl border border-[#E5E5E5] bg-white px-3 py-2 shadow-sm">
                <div className="flex flex-1 flex-col gap-1">
                  <span className="text-xs font-normal leading-[15px] text-[#999999]">
                    {t('editProvider.description')}
                  </span>
                  <textarea
                    className="min-h-[40px] resize-none border-0 bg-transparent p-0 text-[15px] font-medium leading-[18px] tracking-[0.15px] text-[#272727] outline-none placeholder:text-[#999999] focus:border-0 focus:outline-none focus:ring-0"
              ...
              {(provider.listing_type !== undefined || reviewFooterActions) &&
                (reviewFooterActions ? (
                  <div className="flex h-[54px] w-full items-center rounded-2xl border border-[#E5E5E5] bg-white px-3 py-2 shadow-sm">
                    <div className="flex flex-1 flex-col gap-1">
                      <label
                        className="text-xs font-normal leading-[15px] text-[#999999]"
                        htmlFor="provider-listing-type"
                      >
                        {t('editProvider.sectionFieldLabel')}
                      </label>
                      <select
                        aria-label={t('editProvider.sectionFieldLabel')}
                        className="bg-transparent p-0 text-[15px] font-medium leading-[18px] tracking-[0.15px] text-[#272727] outline-none"
                  ...
                  <div className="flex h-[54px] w-full items-center rounded-2xl border border-[#E5E5E5] bg-gray-50 px-3 py-2 shadow-sm">
                    <div className="flex flex-1 flex-col gap-1">
                      ...
                      <span className="text-[15px] font-medium capitalize leading-[18px] tracking-[0.15px] text-[#272727]">
            ...
            <div className="space-y-3">
                  ...
                  <div className="break-words text-left text-[15px] font-medium leading-[18px] tracking-[0.15px] text-[#272727]">
                    {(() => {
                      try {
                        ...
                        return imageCount > 0
                          ? t('editProvider.imagesSelected').replace(
                              '{{count}}',
                              imageCount.toString(),
                            )
                          : t('editProvider.uploadImages');
                      } catch {
              ...
              {!hideSocialInitiatives && (
                ...
                >
                  <div className="flex flex-1 flex-col items-start gap-1">
                    <span className="text-xs font-normal leading-[15px] text-[#999999]">
                      {t('editProvider.socialInitiatives')}
                    </span>
                    <div className="break-words text-left text-[15px] font-medium leading-[18px] tracking-[0.15px] text-[#272727]">
                      {(formData.selectedCommunityServiceIds || []).length > 0
                        ? t('editProvider.initiativesSelected').replace(
                            '{{count}}',
        ...
        <div className="flex flex-col gap-4">
          <button
            ...
            type="button"
            onClick={() =>
              setExpandedSections((prev) => ({ ...prev, providerDetails: !prev.providerDetails }))
            }
          >
          ...
          {expandedSections.providerDetails && (
            <div className="space-y-3">
              ...
              {formData.listingType === 'food' && (
                ...
                >
                  <div className="flex flex-1 flex-col items-start gap-1">
                    <span className="text-xs font-normal leading-[15px] text-[#999999]">Menu</span>
                    <div className="text-[15px] font-medium leading-[18px] text-[#272727]">
              ...
              >
                <div className="flex flex-1 flex-col items-start gap-1">
                  ...
                  <div className="text-[15px] font-medium leading-[18px] text-[#272727]">
                    ...
                    Object.values(formData.openingHours).some((v) => v !== null)
                      ? 'Set'
                      : 'Set opening hours'}
                  </div>
                </div>
