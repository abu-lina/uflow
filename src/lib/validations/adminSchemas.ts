 ...
 */
export const providerReviewUpdateSchema = z
  .object({
  ...
  .refine(
    ...
    {
      message:
        'Rejection reason is required. Please provide feedback explaining why this provider is being rejected.',
      path: ['reviewFeedback'],
    },
  );
      ...
      (val) => {
        ...
        try {
          ...
          if (typeof parsed === 'object') {
            ...
            return (
              Array.isArray(parsed.urls) && parsed.urls.every((u: unknown) => typeof u === 'string')
            );
          }
          return false;
        } catch {
 ...
 */
export const communityServiceReviewUpdateSchema = z
  .object({
    communityServiceId: z.string().uuid('Invalid community service ID format'),
    reviewStatus: z.enum(['approved', 'rejected', 'needs_revision'], {
      errorMap: () => ({
  ...
  .refine(
    ...
    {
      message:
        'Rejection reason is required. Please provide feedback explaining why this community service is being rejected.',
      path: ['reviewFeedback'],
    },
  );
