...
async function testRLS() {
  ...
  // Test 6: Testing business creation based on role
  console.log('\n6. Testing business creation based on role:');
  ...
  const { data: businessData, error: businessError } = await authClient
    .from('businesses')
    .insert({
  ...
  if (businessError) {
    if (roleData?.role === 'business_owner' || roleData?.role === 'admin') {
      console.error('❌ Business creation failed for business_owner/admin:', businessError.message);
    } else {
      console.log('✅ Business creation correctly failed for non-business user:', businessError.message);
    }
  } else {
    if (roleData?.role === 'business_owner' || roleData?.role === 'admin') {
      console.log('✅ Business creation succeeded for business_owner/admin:', businessData);
    } else {
      console.error('❌ Business creation incorrectly succeeded for non-business user:', businessData);
    }
  }
