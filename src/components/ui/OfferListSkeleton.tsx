...
export function OfferListSkeleton() {
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="relative h-[40px] w-full overflow-hidden rounded-2xl bg-gray-200">
      ...
      <div className="space-y-3">
        ...
        <div className="relative overflow-hidden rounded-2xl border border-gray-200 bg-white/50 p-4 shadow-sm">
          <div className="absolute inset-0 -translate-x-full animate-[shimmer_2s_infinite] bg-gradient-to-r from-transparent via-white/60 to-transparent" />
          <div className="space-y-3">
            <div className="h-6 w-32 rounded bg-gray-200" />
            <div className="flex flex-wrap gap-2">
              {Array.from({ length: 4 }).map((_, i) => (
                <div key={i} className="h-8 w-24 rounded-xl bg-gray-200" />
              ))}
            </div>
          </div>
        </div>
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              {Array.from({ length: 6 }).map((_, i) => (
                <div key={i} className="h-8 w-28 rounded-xl bg-gray-200" />
              ))}
            </div>
          </div>
        </div>
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              {Array.from({ length: 8 }).map((_, i) => (
                <div key={i} className="h-8 w-32 rounded-xl bg-gray-200" />
              ))}
            </div>
          </div>
        </div>
