...
export default function SocialProjectPage() {
  ...
  const filteredProjects = communityServices.filter(
    (service) =>
      service.community_service_name.toLowerCase().includes(searchQuery.toLowerCase()) ||
      (service.community_service_description &&
        service.community_service_description.toLowerCase().includes(searchQuery.toLowerCase())),
  );
          ...
          {servicesLoading ? (
            ...
            <div className="grid w-full grid-cols-2 gap-4">
              {filteredProjects.map((service) => {
                const isSelected = (formData.selectedCommunityServiceIds || []).includes(
                  service.community_service_id,
                );
                const imageUrl = getFirstImageUrl(service.community_service_images);
                const donationText =
                  service.donation_count && service.donation_count > 0
            ...
            <div className="flex flex-col items-center justify-center py-8">
              ...
              <p className="text-sm text-gray-500">
                {searchQuery
                  ? t('create.media.noProjectsFound')
                  : t('create.media.noDonationProjectsAvailable')}
              </p>
              {!searchQuery && (
