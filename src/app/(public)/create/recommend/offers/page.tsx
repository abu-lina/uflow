...
export default function SelectOffersPage() {
  ...
  const filteredSuggestedOffers = useMemo(() => {
    ...
    return offers.filter(
      (o) => suggestedIds.includes(o.offer_id) && !formData.offers_ids.includes(o.offer_id),
    );
  ...
  const showCreateOption =
    searchQuery.trim() &&
    !offers.some((o) => normalizeText(o.name_de) === normalizeText(searchQuery.trim()));
          ...
          <div className="flex flex-col gap-6">
            ...
            {isLoading ? (
              ...
              <div className="flex flex-col gap-3">
                    ...
                    {isSelectedExpanded && (
                      <div className="flex flex-wrap gap-2">
                        {filteredSelectedOffers.map((offer) => (
                          <button
                            key={offer.offer_id}
                            className="flex items-center gap-2 rounded-xl border border-primary bg-primary-light px-3 py-2 text-sm font-medium text-primary"
                    ...
                    >
                      <h3 className="text-md font-medium text-[#232323]">
                        {t('create.offers.recommendedFor').replace(
                          '{{category}}',
                          formData.category ? t('create.recommend.category') : '',
                        )}
                      </h3>
                      <Icon
                        aria-hidden="true"
                    ...
                    {isSuggestedExpanded && (
                      <div className="flex flex-wrap gap-2">
                        {filteredSuggestedOffers.map((offer) => (
                          <button
                            key={offer.offer_id}
                            className="flex items-center gap-2 rounded-xl border border-gray-200 bg-white px-3 py-2 text-sm font-medium text-[#232323] transition-colors hover:border-primary hover:bg-primary-light"
                            onClick={() => toggleOffer(offer.offer_id)}
                          >
                            <span>{offer.name_de}</span>
                ...
                {showCreateOption && (
                  <button
                    className="flex items-center gap-2 rounded-xl border border-primary bg-primary-light px-4 py-3 text-sm font-medium text-primary transition-colors hover:bg-primary hover:text-white"
                    disabled={isCreating}
                    onClick={createOfferFromSearch}
                  >
                    <Icon className="h-5 w-5" icon="lucide:plus" />
                    <span>
                      {t('create.offers.addButton').replace('{{query}}', searchQuery.trim())}
                    </span>
                  </button>
                )}
              </div>
