...
export default function SelectNeedsPage() {
  ...
  // Get selected needs
  const selectedNeeds = needs.filter((need) => formData.needs_ids.includes(need.need_id));

  // Split needs into suggested and other (excluding selected)
  const availableSuggestedNeeds = suggestedNeeds.filter(
    (need) => !formData.needs_ids.includes(need.need_id),
  );

  // Split other needs into category-related and unrelated
  const otherNeeds = needs.filter(
    (need) => !suggestedNeedIds.has(need.need_id) && !formData.needs_ids.includes(need.need_id),
  );

  // Category-related other needs (matching category but not in suggested list)
  const categoryRelatedNeeds = otherNeeds.filter((need) => need.category_id === formData.category);

  // Unrelated needs (from other categories or uncategorized)
  const unrelatedNeeds = otherNeeds.filter((need) => need.category_id !== formData.category);
  ...
  const hasExactMatch = categoryFilteredNeeds.some(
    (need) => need.name_de.toLowerCase() === searchLower,
  );
  ...
  const createNeedFromSearch = async () => {
    ...
    if (validation.shouldAutoSelect) {
      ...
      const existingNeed = needs.find((n) => n.name_de === similarName);
      if (existingNeed) {
        ...
        setSearchQuery('');
        toast.info(t('create.needs.autoSelected').replace('{{name}}', existingNeed.name_de), {
    ...
    if (
      ...
      if (!confirmed) {
        ...
        if (firstSimilar) {
          const mostSimilar = needs.find((n) => n.name_de === firstSimilar.name_de);
        ...
        <div className="flex w-full flex-col gap-2">
          ...
          <div className="flex h-[40px] w-full items-center rounded-2xl border-0 bg-white px-[10px] py-[5px]">
            <div className="flex flex-1 items-center gap-3">
              ...
              <input
                className="flex-1 border-0 bg-transparent pl-0 text-xs font-normal leading-[15px] text-[#7C7C7C] outline-none placeholder:text-[#7C7C7C] focus:border-0 focus:outline-none focus:ring-0"
            ...
            <>
              ...
              {filteredSuggestedNeeds.length > 0 && (
                <div className="mb-6 rounded-2xl border border-gray-200 bg-white/50 p-4 shadow-sm">
                  ...
                  >
                    <h3 className="text-md font-medium text-[#232323]">
                      {t('create.needs.recommendedFor').replace(
                        '{{category}}',
              ...
              {filteredCategoryRelatedNeeds.length > 0 && (
                <div className="mb-6 rounded-2xl border border-gray-200 bg-white/50 p-4 shadow-sm">
                  ...
                  >
                    <h3 className="text-md font-medium text-[#232323]">
                      {t('create.needs.moreNeeds') || 'Weitere Bedürfnisse'} (
                      {filteredCategoryRelatedNeeds.length})
                    </h3>
                    <Icon
                      className={`h-5 w-5 text-gray-600 transition-transform duration-200 ${
