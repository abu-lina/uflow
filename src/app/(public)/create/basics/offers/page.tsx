...
export default function SelectOffersPage() {
  ...
  const filteredSelectedOffers = useMemo(
    () => selectedOffers.filter((offer) => offer.name_de.toLowerCase().includes(searchLower)),
    [selectedOffers, searchLower],
  );

  const filteredSuggestedOffers = useMemo(
    () =>
      availableSuggestedOffers.filter((offer) => offer.name_de.toLowerCase().includes(searchLower)),
    [availableSuggestedOffers, searchLower],
  );

  const filteredOtherOffers = useMemo(
    () => otherOffers.filter((offer) => offer.name_de.toLowerCase().includes(searchLower)),
    [otherOffers, searchLower],
  );

  // Memoize exact match check
  const hasExactMatch = useMemo(
    () => categoryFilteredOffers.some((offer) => offer.name_de.toLowerCase() === searchLower),
  ...
  const createOfferFromSearch = useCallback(async () => {
    ...
    // Check rate limit
    if (!checkLimit()) {
      const secondsRemaining = resetTime ? Math.ceil((resetTime - Date.now()) / 1000) : 60;
      toast.error(
    ...
    if (validation.shouldAutoSelect) {
      ...
      if (existingOffer) {
        ...
        setSearchQuery('');
        toast.info(t('create.offers.autoSelected').replace('{{name}}', existingOffer.name_de), {
    ...
    if (
      ...
      validation.similarItems &&
      validation.similarItems.length > 0
    ) {
  ...
  const deleteOffer = useCallback(
    async (offerId: string, offerName: string) => {
      if (!user) return;
        ...
        >
          <div className="flex flex-col gap-6">
            ...
            <div className="flex w-full flex-col gap-2">
              {/* Search Bar */}
              <div className="flex h-[40px] w-full items-center rounded-2xl border-0 bg-white px-[10px] py-[5px]">
                <div className="flex flex-1 items-center gap-3">
                  <Icon className="size-6 shrink-0 text-[#1B1D1D]" icon="lucide:search" />
                  <input
                    className="flex-1 border-0 bg-transparent pl-0 text-base font-normal text-gray-600 outline-none placeholder:text-sm placeholder:text-gray-500 focus:border-0 focus:outline-none focus:ring-0"
                  ...
                  {searchQuery && (
                    <button
                      className="text-gray-500 transition-colors hover:text-[#232323]"
            ...
            {isLoading ? (
              ...
              <div className="flex flex-col gap-3">
                      ...
                      <div className="animate-in fade-in flex flex-wrap gap-2 duration-200">
                        {filteredSelectedOffers.map((offer) => (
                          <div key={offer.offer_id} className="relative">
                            <button
                              className="inline-flex rounded-xl border border-primary bg-primary-light px-4 py-2 pr-3 text-left text-content-heading transition-all duration-200"
                            ...
                            {offer.created_by === user?.id && (
                              <button
                                className="absolute -right-2 -top-2 rounded-full bg-red-500 p-1 text-white shadow-sm transition-colors hover:bg-red-600 disabled:opacity-50"
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {similarOffers.map((offer) => (
                            <button
                              key={offer.offer_id}
                              className="rounded-md border border-yellow-300 bg-yellow-100 px-2 py-1 text-xs text-yellow-800 transition-colors hover:bg-yellow-200"
                              onClick={() => {
                ...
                {showCreateOption && (
                  <div>
                    <h3 className="mb-3 text-sm font-medium text-[#232323]">
                      {t('create.offers.createNew')}
                    </h3>
                    <button
                      className="inline-flex items-center gap-2 rounded-xl border border-primary bg-primary px-4 py-2 text-white transition-all duration-200 hover:bg-primary-dark active:bg-primary-darker disabled:opacity-50"
                      disabled={isCreating}
                ...
                {filteredSuggestedOffers.length > 0 && (
                  <div className="rounded-2xl border border-gray-200 bg-white/50 p-4 shadow-sm">
                    ...
                    >
                      <h3 className="text-md font-medium text-[#232323]">
                        {t('create.offers.recommendedFor').replace(
                          '{{category}}',
                    ...
                    {isSuggestedExpanded && (
                      <div className="animate-in fade-in flex flex-wrap gap-2 duration-200">
                        {filteredSuggestedOffers.map((offer) => (
                          <button
                            key={offer.offer_id}
                            className="inline-flex rounded-xl border border-gray-200 bg-white px-4 py-2 text-left text-[#232323] transition-all duration-200 hover:border-gray-300 hover:bg-gray-50"
                            onClick={() => toggleOffer(offer.offer_id)}
                          >
                            <span className="text-sm font-medium">{offer.name_de}</span>
                          </button>
                ...
                {hasMoreOffers && !searchQuery && filteredOtherOffers.length > 0 && (
                  <div className="flex justify-center pt-4">
                    <button
                      className="inline-flex items-center gap-2 rounded-xl border border-gray-200 bg-white px-6 py-2 text-[#232323] transition-all duration-200 hover:border-gray-300 hover:bg-gray-50 disabled:opacity-50"
                      disabled={isLoadingMore}
                      onClick={loadMoreOffers}
                    >
                      <Icon
                        className="h-4 w-4"
                        icon={isLoadingMore ? 'lucide:loader-2' : 'lucide:chevron-down'}
                      />
                      <span className="text-sm font-medium">
                        {isLoadingMore
