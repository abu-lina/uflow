...
/**
 * Needs selection sub-page for community service edit.
 *
 * Adapts the provider edit needs sub-page (Plan 083 — M3):
 *  - Queries community_services table (not providers)
 *  - Uses community_service_id column (not provider_id)
 *  - Uses admin_cs_edit_needs_${id} localStorage key (not admin_edit_needs_${id})
 */
export default function CsEditNeedsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const loadCurrentNeeds = async () => {
      try {
        ...
        const stored = localStorage.getItem(`admin_cs_edit_needs_${communityServiceId}`);
        if (stored) {
  ...
  const toggleNeed = (needId: string) => {
    setSelectedNeedIds(prev => {
      ...
      localStorage.setItem(`admin_cs_edit_needs_${communityServiceId}`, JSON.stringify(newSelection));
      return newSelection;
    });
  };
