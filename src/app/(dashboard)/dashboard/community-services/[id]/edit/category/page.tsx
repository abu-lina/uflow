...
/**
 * Category selection sub-page for community service edit.
 *
 * Adapts the provider edit category sub-page (Plan 083 — M3):
 *  - Queries community_services table (not providers)
 *  - Uses community_service_id column (not provider_id)
 *  - Uses admin_cs_edit_category_${id} localStorage key (not admin_edit_category_${id})
 */
export default function CsEditCategoryPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const loadCurrentCategory = async () => {
      try {
        // CS-specific localStorage key (admin_cs_edit_category_${id})
        const stored = localStorage.getItem(`admin_cs_edit_category_${communityServiceId}`);
        if (stored) {
  ...
  const handleCategorySelect = (categoryId: string) => {
    ...
    localStorage.setItem(`admin_cs_edit_category_${communityServiceId}`, categoryId);
    router.back();
  };
