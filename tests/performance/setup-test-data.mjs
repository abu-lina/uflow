 ...
 * 
 * Usage:
 *   node tests/performance/setup-test-data.mjs [--cleanup]
 */
...
if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY) {
  ...
  console.error(`   File path: ${envFile}`);
  process.exit(1);
}

// Verify key format (service role keys typically start with 'eyJ')
if (!SUPABASE_SERVICE_KEY.startsWith('eyJ')) {
  console.warn('⚠️  Warning: Service role key format looks incorrect');
  console.warn('   Service role keys should start with "eyJ" (JWT format)');
}
...
if (import.meta.url === `file://${process.argv[1]}` || import.meta.url.endsWith('setup-test-data.mjs')) {
  main().catch(error => {
