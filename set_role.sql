...
-- For example: UPDATE users SET role = 'owner' WHERE email = 'your@email.com';

-- If you know the user_id (from auth.users), you can use this instead:
-- UPDATE users SET role = 'owner' WHERE user_id = 'user-id-here';
...
-- To make all existing users owners (be careful with this!):
