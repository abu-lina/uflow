...
| OA-1 | D7/CR fix | Admin community service edit route `/dashboard/community-services/[id]/edit` does not exist yet. `AdminCommunityServiceDetailButtons` is implemented and targets this URL. Admin clicking the button will encounter a 404 until the route is built. Requires a future admin dashboard plan to implement the CS edit page. | User/Planner | When admin CRUD for community services is prioritized. |
...
| MW-3 | Admin user opens any community service → admin action button visible; clicking routes to `/dashboard/community-services/[id]/edit` (⚠️ route not yet built — see OA-1) | Pending UAT |
