...
| 2026-04-29 | **[RECONCILED] Plan 114 Phase 0-prime implemented (v0.11.1)**: Migration baseline squash complete. `001_baseline.sql` (158 KB prod-derived DDL), `002_seed.sql` (62 KB INSERT-based), `003_phase0_schema_hygiene.sql` (2.4 KB — dropped 7 redundant indexes, removed duplicate trigger, added 2 composite indexes) applied to prod via MCP. 84 historical migrations archived. `supabase_migrations.schema_migrations` bootstrapped on prod with 001/002/003 registered. Problem Areas 12 (partial) and 13 resolved. Recommendations 5 and 6 complete. | Completed implementation becomes architectural fact. Prod now has migration tracking for the first time. | Plan 114 / v0.11.1 |
...
13. **~~Migration management gap (ADR-114)~~** ✅ **RESOLVED (v0.11.1)**: Prod now has `supabase_migrations.schema_migrations` with 001/002/003 registered. Local uses the same 3-file baseline chain. Historical 84 migrations archived to `supabase/migrations/archive/`. All future migrations are forward-only from baseline. Dev still has 4 stale timestamp migrations (acceptable — dev resets will pick up the new chain). See ADR-114, Plan 114.
...
5. ~~**[PRIORITY] Execute migration baseline squash (ADR-114)**~~ ✅ **DONE (v0.11.1)**: `001_baseline.sql` + `002_seed.sql` + `003_phase0_schema_hygiene.sql` applied. 84 historical migrations archived. All future migrations forward-only from baseline.
6. ~~**Establish migration tracking on prod**~~ ✅ **DONE (v0.11.1)**: `supabase_migrations.schema_migrations` bootstrapped on prod with 001/002/003 registered via MCP.
7. **[NEXT] Execute Plan 114 Phase 1 (F-9: Environment Alignment)**: Resolve `consent_logs` (local+dev only) vs `deletion_logs` (prod only) divergence. Baseline is now established; environment parity achievable.
8. **[NEXT] Execute Plan 114 Phase 2 (F-3: Data Coherence)**: Drop `barakah_effects TEXT[]` outright. Make boolean columns the sole filter data source. Requires Phase 1 complete.
