-- ============================================================
-- Migration: Tri-state halal attestations in admin_update_provider
-- Date: 2026-09-05
-- Problem: Migration 124 writes the extension-table attestation
--   columns with COALESCE((v_->>'col')::boolean, false) on INSERT
--   and COALESCE(EXCLUDED.col, tbl.col) on UPDATE. After migration
--   129 made no_alcohol/no_pork/no_gambling nullable ("not sure" =
--   NULL), an explicit null in the payload is clamped to false on
--   insert, and on update EXCLUDED.col is the already-coalesced
--   false so the current row value can never be NULL either. Every
--   admin "not sure" save writes false, which the halal gate reads
--   as "declared non-halal" and rejects (Plan 255, AC6.3).
-- Fix: replace the COALESCEs with the same key-presence CASE
--   pattern 124 established for nullable strings, on the INSERT
--   VALUES and the ON CONFLICT DO UPDATE branch of both
--   food_providers and store_providers (6 sites). Everything else
--   in the function body is preserved verbatim from 124.
-- ============================================================
...
COMMENT ON FUNCTION public.admin_update_provider IS
  'Fix nullable string fields: use CASE WHEN ? instead of COALESCE so null clears the value.';
