-- ============================================================
-- Migration: Plan 151 — Admin location upsert in admin_update_provider RPC
-- Date: 2026-06-06
-- Updates the admin_update_provider RPC to handle locations
-- with an upsert pattern (preserve existing location_id values).
-- ============================================================
...
BEGIN
  ...
  -- Upsert locations: preserve existing IDs, update if present, insert if new, delete if not in payload
  IF p_data ? 'locations' THEN
    ...
    -- Delete locations that are in the DB but not in the payload
...
COMMENT ON FUNCTION public.admin_update_provider IS
  'Plan 151: Atomic multi-table update for provider edit page. Accepts JSONB payload with providers, food_providers, store_providers, menu_items, delivery_links, community_service_ids, and locations sub-objects. Locations use upsert pattern to preserve existing IDs.';
