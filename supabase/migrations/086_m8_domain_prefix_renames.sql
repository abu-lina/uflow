-- =============================================================================
-- Migration 086: M-8 — Domain-Prefix Table Renames
-- Plan: PLAN-116  Origin: 118  UUID: e7a3f1c9
-- Renames:
--   provider_menu    → food_menu
--   provider_catalog → store_catalog
-- Rationale: Align with extension table naming convention (food_providers,
--   store_providers, ummah_providers). Domain-prefix names are immediately
--   self-documenting: "food_menu" tells you exactly what it holds.
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='provider_menu') THEN
    ALTER TABLE public.provider_menu RENAME TO food_menu;
  END IF;
END $$;

DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='provider_catalog') THEN
    ALTER TABLE public.provider_catalog RENAME TO store_catalog;
  END IF;
END $$;

-- ---------------------------------------------------------------------------
-- Step 2: Rewrite search_food_menu_items — provider_menu → food_menu
-- ---------------------------------------------------------------------------
...
COMMENT ON FUNCTION public.search_food_menu_items(text, integer) IS
  'Searches food menu items with exact and prefix text matching and returns deduplicated dish names with approved food-provider counts.';

-- ---------------------------------------------------------------------------
-- Step 3: Rewrite search_provider_items — provider_menu → food_menu,
--         provider_catalog → store_catalog
-- ---------------------------------------------------------------------------
...
COMMENT ON FUNCTION public.search_provider_items(text, text, uuid, integer, integer) IS
  'Unified item search across food_menu and store_catalog with listing_type/provider filters.';
