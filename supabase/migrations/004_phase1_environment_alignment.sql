...
BEGIN
  IF EXISTS (
    ...
    IF EXISTS (
      ...
      RAISE EXCEPTION 'Cannot set consent_logs.user_id to NOT NULL: existing NULL values detected';
    END IF;
  ...
  IF EXISTS (
    ...
    IF EXISTS (
      ...
      RAISE EXCEPTION 'Cannot set consent_logs.consent_type to NOT NULL: existing NULL values detected';
    END IF;
...
BEGIN
  IF NOT EXISTS (
    ...
    JOIN pg_class t ON t.oid = c.conrelid
    JOIN pg_namespace n ON n.oid = t.relnamespace
    WHERE c.conname = 'deletion_logs_user_id_fkey'
      AND n.nspname = 'public'
      AND t.relname = 'deletion_logs'
  ) THEN
    ALTER TABLE public.deletion_logs
      ADD CONSTRAINT deletion_logs_user_id_fkey
      FOREIGN KEY (user_id)
      REFERENCES auth.users(id)
      ON DELETE SET NULL;
  END IF;
...
COMMENT ON COLUMN public.deletion_logs.user_id IS
  'Nullable FK to auth.users(id). ON DELETE SET NULL preserves deletion audit records after account removal.';
